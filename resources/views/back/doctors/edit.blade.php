...
<h2 class="mx-7 font-semibold px-3 py-3 text-center text-gray-700 border border-gray-200 rounded-lg bg-gray-50">Edit Doctor Information</h2>
...
<form id="data-form" class="lg:ms-auto mt-1 mx-auto text-center" action="{{ route('doctors.update', $doctor->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <div class="py-7 px-7 rounded-md bg-white">
        <div class="grid md:grid-cols-2 grid-cols-1 gap-6">
            ...
            <input disabled value="{{ $doctor->slug }}" type="text" id="slug" name="slug" placeholder="Slug *" class="w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:border-blue-700 ">
            ...
            <!-- Input field for image will not have the value attribute as it's usually not pre-filled -->
            <input type="file" id="image" name="image" placeholder="Select Doctors Photo" class="w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:border-blue-700">
...
<tr class="bg-white border-b hover:bg-gray-50">
    <th scope="row" class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap">
        <input type="text" value="{{ $day }}" class="w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:border-blue-700" readonly>
        <input value="{{ $dayKeys[$index] }}" type="hidden" name="day[{{ $index }}][day]" readonly>
    </th>
    <td class="px-6 py-3">
        <input value="{{ $doctor->timetable[$index]->start_time ?? '' }}" type="time" class="w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:border-blue-700" name="day[{{ $index }}][start_time]" id="start_time_{{ $index }}">
    </td>
    <td class="px-6 py-3">
        <input value="{{ $doctor->timetable[$index]->end_time ?? '' }}" type="time" class="w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:border-blue-700" name="day[{{ $index }}][end_time]" id="end_time_{{ $index }}">
    </td>
    <td class="px-6 py-3">
        <input value="{{ $doctor->timetable[$index]->remarks ?? '' }}" type="text" class="w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:border-blue-700" name="day[{{ $index }}][remarks]" id="remarks_{{ $index }}">
    </td>
</tr>
@endforeach
