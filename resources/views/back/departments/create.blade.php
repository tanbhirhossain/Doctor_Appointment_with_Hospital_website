...
<h2 class="mx-7 font-semibold px-3 py-3 text-center  text-gray-700 border border-gray-200 rounded-lg bg-gray-50">Department Information</h2>
...
<form id="data-form" class="lg:ms-auto mt-1 mx-auto text-center" action="{{ route('departments.store') }}" method="POST" enctype="multipart/form-data">@csrf
    <div class="py-7 px-7 rounded-md bg-white">
            ...
            <div class="grid md:grid-cols-2 grid-cols-1 gap-6">
              ...
              <div class="md:col-span-2">
                <textarea name="description" rows="5" cols="" placeholder="" class="w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:border-blue-700">{{ old('description') }}
                   
                </textarea>
              </div>
              <div>
                <label for="image">Image</label>
                <input value="{{ old('image') }}" type="file" id="image" name="image" placeholder="Select Photo" class="w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:border-blue-700">
              </div>
              <div>
                <label for="image">Icon *</label>
                <input value="{{ old('icon') }}" type="file" id="icon" name="icon" placeholder="Select Icon" class="w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:border-blue-700">
            </div>
              <select id="department_id" name="department_id" class="w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:border-blue-700">
                ...
                @foreach ($departments as $item)
                    <option value="{{ $item->name }}" {{ old('parent_id') == $item->name ? 'selected' : '' }}>{{ $item->name }}</option>
                @endforeach
              ...
              <button type="submit" class="text-white bg-gradient-to-r from-green-400 via-green-500 to-green-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-green-300 dark:focus:ring-green-800 shadow-lg shadow-green-500/50 dark:shadow-lg dark:shadow-green-800/80 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">Create Department</button>
            </div>
